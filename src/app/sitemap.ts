...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const baseUrl = 'https://hyunjoong.com';
  ...
  // Blog post pages
  const blogPages: MetadataRoute.Sitemap = postIdentifiers.map(({ slug, lang }) => ({
  ...
  // Project pages
  const projectPages: MetadataRoute.Sitemap = projectIdentifiers.map(({ slug, lang }) => ({
